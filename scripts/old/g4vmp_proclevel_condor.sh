...
export SCRIPT_VERSION=2017-02-06
#
...
export OUTPUTTOP="/pnfs/geant4/persistent/rhatcher/g4vmp"
...
export ARTG4TK_MRB=/pnfs/geant4/persistent/rhatcher/g4vmp/mrb_work_area.tar.gz
export ARTG4TK_VERSION="v0_01_00"
export ARTG4TK_QUAL="e7:prof"
...
# ART process name ... arbitrary?
export ARTPNAME="ABC"

# [/path/]filename[,minu[,maxu]]  minu,maxu 1-based
export MULTI_UNIVERSE_SPEC="/geant4/app/rhatcher/multiverse170208_Bertini.fcl"
...
Purpose:  Run 'artg4tk' w/ the Geant4 Varied Model Parameter setup
  ...
  ${b0} --output <output-path> [other options]
     ...
     -n | --nevts <nevents>     # of events to generate (single job) [${NEVENTS}]
...
#if [ -t 1 ]; then
if [ -d /pnfs/geant4/persistent/rhatcher/g4vmp/ ]; then
...
if [ "\${copt}" != "IS-A-REAL-PATH" ]; then
  ...
  if [ ! -f \${TARBALL_BASE} ]; then
     echo -e "\${OUTRED}\"ifdh cp\" failed; try simple \"cp\" to get ${TARBALL_BASE}\${OUTNOCOL}"
...
function report_config_summary()
{
  ...
  echo "   projectile:    ${PRIMARY_PDG} [ ${PRIMARY_PX}, ${PRIMARY_PY}, ${PRIMARY_PZ} ] GeV/c"
  echo " "
}
##############################################################################
...
export DESTDIR=${OUTPUTTOP}/${CONFIGBASE}/${JOBID4}
echo -e ""
...
localList="${CONFIG}.artg4tk.root ${CONFIG}.fcl ${LOCAL_MULTI_UNIVERSE_FILE}"
